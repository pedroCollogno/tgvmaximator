...
def user_inputs_preprocessing(selected_departure_city, selected_arrival_city):
...
def ping_sncf(departing_day, departure_city_code, arrival_city_code):
    ...
    res = requests.get(url + '/FR' + departure_city_code + '/FR' + arrival_city_code + '/' + departing_day + options)

    jsoned_data = json.loads(res.content)

    return jsoned_data

def api_call(departure_city, arrival_city, departing_day):

    departure_city_code, arrival_city_code = user_inputs_preprocessing(departure_city, arrival_city)

    jsoned_data = ping_sncf(departing_day, departure_city_code, arrival_city_code)
