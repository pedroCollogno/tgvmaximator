...
def apiCall(departure_city,arrival_city,required_day, options='extendedToLocality=true&onlyDirectTrains=true'):
    res = requests.get('https://www.oui.sncf/calendar/cdp/api/proposals/v3/outward/FR' + departure_city +'/FR' + arrival_city + '/' + required_day.strftime("%Y-%m-%d") + '/12-HAPPY_CARD/2/fr/fr?' + options)
    ...
    for billet in jsoned_data:
        ...
        billet_info = {'id':billet['id'],'departure_time': datetime.min, 'arrival_time': datetime.max, 'price': 10000000000000000, 'is_tgvmax': False, 'is_trakos': True}
